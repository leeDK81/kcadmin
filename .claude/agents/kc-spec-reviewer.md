...
각 카드 편집기의 입력 필드가 **`context/card-types.md`**(단일 진실원 — 기획서와 충돌 시 이 파일이 우선한다, `context/decisions.md` 헤더 원칙)와 일치하는지 확인한다. 카드 유형: Concept·Risk-type·Rule·Evidence·Policy·Playbook 6종 + Case(Phase 1.5 이후, 목업 범위 외).

### A-1. Case 0~4 매트릭스 — 조회형 게이트 + KC매칭 × Playbook감지
...
- [ ] Standalone 답변 가이드가 **선택사항**(비워두면 Clark 기본 안내 문구 사용, 20자 이상 필수 등 삭제된 스펙 없음) 정책과 일치하는가? UI 상세(textarea 위치·배너 문구·링크 대상 등)는 `kc-ui-reviewer`가 이미 확인하므로 여기서 다시 나열하지 않는다.
...
- [ ] `17_system-data-guide.html`의 Evidence 표기가 단일 유형(공인 외부 통계 기반)으로 되어 있는가? (보닥통계·프롬에이지 기반 표기 없어야 함)
...
카드 유형별 공개범위가 편집기에 **표시되지 않아야** 하며, 목록 화면에서만 읽기 전용 배지로 표시된다. **카드 유형별 공개범위 값(고객 공개/공통 기준/내부 전용)은 `context/card-policy.md`가 단일 진실원**이다.
...
> **단일 진실원은 `guides/insurance-domain.md`다(사본 두지 않음).** 이 파일에 독자 T코드 표를 유지하지 말 것. 검수 시 반드시 `guides/insurance-domain.md`의 최신 표(T01~T32 중 23개 활성)를 직접 열어 대조한다.

### F. Rule 조건 빌더 — 필드·연산자 정합성

조건 빌더에서 선택 가능한 source·필드·연산자·등급명이 **`context/card-types.md`**(단일 진실원)와 정확히 일치하는지 확인한다.
...
> **이 섹션은 "정책 표 자체가 기획서·`card-policy.md`와 일치하는지"만 본다.** 코드가 그 정책을 실제로 구현했는지(배지·블로킹 로직 등)는 `kc-code-reviewer`의 몫이다 — 같은 사실을 두 관점에서 보는 것이지 중복이 아니다. **카드 간 연결 방향·필수 여부 값 자체는 `context/card-policy.md`가 단일 진실원**이다.
...
## 19_faq-rag.html 검수 포인트

**Clark 서비스 전용 FAQ RAG 관리 화면 정합성:**

- [ ] 상단 안내 배너: "계약 특정적 질문(개인 보장금액·청구 가능 여부 등)은 등록 금지" 취지 문구 있는가? (주의: "약관·보장 관련 Q&A 등록 금지"는 계약 무관 판단 노하우까지 막는 것으로 오독되는 폐기된 표현이다 — `context/answer-logic.md` "FAQ RAG 등록 기준" 표 참조)
- [ ] 목록 테이블: 상태 필터 **2종** (전체/인덱스등록됨/초안) 있는가? (검수대기·반려됨 없어야 함)
- [ ] 등록 패널: LLM 초안 자동 생성 버튼 없는가? (운영자 직접 입력만)
- [ ] 등록 패널 필드: Q·A(필수) + 출처·⚠️유의사항(선택) 4개 있는가? (Q·A 2개만 있으면 오류)
