...
## 2026-07-20(5차) — Playbook "데이터 미보유 감지 조건" 카드 게이트 완전 폐지 — 버튼별 노출 조건으로 전면 통합

바로 아래 항목(4차)에서 체크박스로 단순화한 직후, 사용자가 더 근본적인 질문을 던졌다 — "그런데 null 유무 체크 자체가 필요없지 않을까? 상담신청은 마이데이터 연결이 전제고, 마이데이터 연결은 회원가입이 전제다."

즉 상담신청에 이르는 경로 자체가 이미 회원가입→마이데이터연결이라는 계단식 전제조건을 갖고 있으므로, Playbook이 사용자 상태를 미리 감지해서 카드 단위로 "이 사람에겐 이 CTA, 저 사람에겐 저 CTA"를 사전 분기할 필요가 없다는 통찰이었다. 다만 KC Engine의 답변 생성 로직(마이데이터 없으면 진단 자체가 불가)과 Case 0 강제 CTA(로그인 안 된 조회형 발화에 로그인 버튼을 답변 자체에 강제로 붙이는 것)는 별개 층위라 그대로 유지해야 한다는 점도 함께 정리했다.

이어서 사용자가 구체적인 구현 방향까지 제시했다 — "편집기 상단의 카드 전체 감지 조건을 없애고, 전환 액션 버튼을 만들 때 각 버튼에 노출 조건을 걸면 되지 않냐. 그러면 버튼을 N개 만들어도 null 값 여부에 따라 노출되면 된다." 4차에서 프롬에이지 전용으로 좁게 만들었던 버튼별 체크박스를, PROFILE·마이데이터까지 포함한 범용 4단 select(상시 노출/PROFILE 없음일 때만/마이데이터 없음일 때만/프롬에이지 없음일 때만)로 일반화하고, 편집기 상단에 있던 "데이터 미보유 감지 조건" 카드(라디오 3개 + 우선순위 규칙)를 완전히 삭제했다.

결과적으로 Playbook은 이제 **키워드 매칭만으로 감지**되고, 매칭된 카드 안에서 어떤 버튼이 함께 뜨는지만 사용자의 실시간 null 상태로 결정된다. 조건들이 서로 다른 필드(PROFILE/마이데이터/프롬에이지)를 독립적으로 보기 때문에 카드 레벨 우선순위 조정도 더 이상 필요 없어져, 이날 이전에 만들었던 "PROFILE없음 > 마이데이터없음" 우선순위 규칙까지 함께 폐지됐다.

**동기화한 파일:**
- `context/answer-logic.md` — "Playbook 데이터 미보유 감지 조건" 절 전면 재작성, "Clark → KC Engine 전달 데이터" 표와 "상태별 답변 흐름" 4개 블록의 Playbook 컬럼을 버튼 노출 조건 기준으로 갱신
- `context/card-types.md` — Playbook 편집기 "7-card 구조" → "6-card 구조"(카드 하나 삭제), Card② 설명에 버튼별 노출 조건 select 반영
- `context/matching-policy.md` — Playbook 매칭 절의 "0단계 우선순위 필터" 삭제, 카드 감지는 키워드만으로 결정됨을 명시
- `mockups_v2/16_card-editor-playbook.html` — "데이터 미보유 감지 조건" 카드 블록 전체 삭제, Card② 추가 버튼에 4단 노출 조건 select 신규
- `mockups_v2/13_answer-logic.html` — 로직 스펙 문서의 상태별 흐름 4곳 + "Playbook 데이터 미보유 감지 조건" 표를 버튼 노출 조건 모델로 교체
- `policy/06_field-data.html` — Playbook 필드 표에서 `missingDataConditions` 필드 삭제, `CTAButton.exposureCondition` 신규 필드 추가

**수정 파일:** `context/answer-logic.md`, `context/card-types.md`, `context/matching-policy.md`, `mockups_v2/16_card-editor-playbook.html`, `mockups_v2/13_answer-logic.html`, `policy/06_field-data.html`, `context/decisions.md`

---

## 2026-07-20(4차) — Card② 추가 버튼 "노출 조건" select → 체크박스 단순화

바로 아래 항목(3차)에서 만든 추가 버튼 "노출 조건" select(상시 노출/프롬에이지 미연동 시만)에 대해, 사용자가 "상단 데이터 미보유 감지 조건은 null 값 여부로 카드 전체를 한 번만 판단하는데, 그걸 버튼 단위에 또 설정하게 만드는 건 개발 입장에서 까다롭지 않냐"고 지적했다.

맞는 지적이었다 — 상단 카드 게이트는 "이 카드가 매칭 후보인가"를 한 번만 판단하는 단순 구조인 반면, 버튼별 노출 조건은 카드가 매칭된 뒤에도 버튼 N개를 순회하며 각각 재평가해야 해서 판단 시점과 구조가 다르다. 게다가 실제로 쓰이는 조건은 프롬에이지 하나뿐인데 select로 만들어 마치 조건 종류가 늘어날 것처럼 확장형으로 설계해뒀던 것도 불필요한 복잡도였다.

select를 "☑ 프롬에이지 미연동 시에만 노출" 체크박스 하나(boolean)로 단순화 — `contents/06_playbook/playbooks.md` PB02~04 실 콘텐츠가 이미 쓰고 있던 단순 플래그 패턴과 정확히 일치한다. 문서에도 이 체크박스가 새 판단 로직이 아니라 KC Engine이 매 요청마다 이미 계산해두는 promage null 여부(Rule 평가 범위 결정에도 쓰이는 값)를 버튼 렌더링 시점에 한 번 더 읽는 것뿐이라는 점을 명시해, 개발팀이 별도 조건 판단 시스템을 새로 만들 필요가 없다는 걸 분명히 했다.

**수정 파일:** `context/answer-logic.md`, `context/card-types.md`, `mockups_v2/16_card-editor-playbook.html`, `context/decisions.md`

---

## 2026-07-20(3차) — "프롬에이지없음" Playbook 게이트 폐지 + 조건부 보조 버튼 신규 도입

사용자가 앞서 만든 `contents/html/06_playbook.html` 샘플을 다시 보다가 "우리 사업은 결국 상담 신청을 받는 것이고, 프롬에이지는 없이도 상담신청이 가능해야 한다 — 프롬에이지는 옵션인데 데이터 미보유 감지 조건이 정책적으로 안 맞는 것 같다"고 지적했다.

확인 결과 앞서(2026-07-20 1차 커밋) 만든 "PROFILE없음 > 마이데이터없음 > 프롬에이지없음" 우선순위 규칙이 문제였다. PROFILE·마이데이터는 없으면 KC 진단 자체가 아예 불가능한 진짜 필수 게이트(상태①②는 KC 체인 진입 불가/제한)인 반면, 프롬에이지는 `context/card-types.md` "프롬에이지 연동 설계"에 이미 Step 6~7 **"더 정밀한 진단"** 선택 단계로 정의돼 있고, 상태③(프롬에이지 null)도 KC 구조화 답변을 온전히 받는다(PROMAGE 선택 조건만 스킵) — 진단도 상담신청도 막지 않는 순수 옵션이다. 기존 CTA 강제노출 규칙(2026-07-07)도 상태①②만 대상이고 상태③은 처음부터 제외돼 있었던 것과도 같은 맥락. "프롬에이지없음"을 PROFILE·마이데이터와 같은 급의 게이트로 두면, 매칭 승자가 될 때마다 상담신청 성격의 일반 Playbook을 밀어내고 "프롬에이지 연동하기" CTA만 노출될 위험이 있어 사업 원칙과 충돌한다는 데 동의해 데이터 미보유 감지 조건 목록에서 완전히 제거(상관없음/PROFILE없음/마이데이터없음 3개만 유지).

제거를 반영하던 중 실제 저작된 콘텐츠 `contents/06_playbook/playbooks.md`의 PB02(암보험)·PB03(심뇌혈관)·PB04(간병보험) 상담 전환 카드를 다시 확인하니, 이미 "기본 버튼(consult, 상담신청)은 항상 노출 + 프롬에이지 연동 버튼(promage_connect)은 프롬에이지 미연동 시에만 추가로 노출"이라는 정확히 올바른 패턴으로 저작돼 있었다 — `mockups_v2/16_card-editor-playbook.html` 편집기 목업이 이걸 "카드 전체를 좌우하는 단일 게이트 조건"으로 잘못 단순화해뒀던 것이며, 실 콘텐츠와 편집기 스펙이 애초부터 어긋나 있었던 것을 이번에 발견했다. 사용자에게 "게이트 목록에서 완전 제거 후 인라인 문구로만 처리" vs "PB02~04처럼 보조 버튼 조건부 노출로 부활" 중 선택을 요청했고, 후자로 확정 — Card②(리드 전환 레이어) 추가 버튼마다 "노출 조건"(상시 노출 / 프롬에이지 미연동 시만) select를 신규 추가해, 편집기에서도 PB02~04와 동일한 패턴("기본 상담신청 버튼 유지 + 프롬에이지 유도는 조건부 추가 버튼")을 만들 수 있도록 반영했다.

**수정 파일:** `context/answer-logic.md`, `context/card-types.md`, `context/matching-policy.md`, `mockups_v2/16_card-editor-playbook.html`, `context/decisions.md`
...
처음엔 각 조건의 정의 자체를 AND-NOT으로 재정의(예: "마이데이터없음" = profile 있음 AND mydata null)하는 안을 검토했으나, 사용자가 "PROFILE 미감지 시 PROFILE Playbook만 노출"이라는 우선순위 방식을 제안 — 이를 3단계 전체로 일반화해 **PROFILE없음 > 마이데이터없음 > 프롬에이지없음** 우선순위로 여러 조건이 동시 매칭돼도 가장 앞단(사용자가 먼저 해결해야 할) 조건 하나만 CTA로 남기고 나머지는 억제하는 방식으로 확정했다. 조건 자체의 정의는 단순 필드 null 체크로 그대로 두고, "매칭 후 승자 결정" 단계에서 우선순위로 정리하는 쪽이 더 단순하다고 판단.

`mockups_v2/16_card-editor-playbook.html` 편집기의 데이터 미보유 조건 UI도 체크박스(카드 하나에 여러 개 선택 가능한 것처럼 보이던 "중복 선택 가능" 문구)에서 **라디오 버튼**(카드당 조건 1개)으로 전환해 편집 의도를 명확히 했다. 조건 간 우선순위 규칙 자체는 UI가 아니라 매칭 엔진 동작이라 안내 배너 문구에 함께 명시.

**수정 파일:** `context/answer-logic.md`, `context/card-types.md`, `context/matching-policy.md`, `mockups_v2/16_card-editor-playbook.html`, `context/decisions.md`

---

## 2026-07-14 — FAQ RAG 편집기(19_faq-rag.html) 질문(Q) 필드를 칩 입력으로 수정 — Concept 동의어 패턴과 통일

사용자가 `contents/html/09_faq-rag.html`(실 콘텐츠 미리보기)과 `mockups_v2/19_faq-rag.html`(운영자 등록 UI 목업)의 질문(Q) 필드 구조가 서로 다르다고 지적 — 확인 결과 목업은 단일 textarea에 "질문1 / 질문2" 식으로 "/" 구분자로 이어붙이는 방식인데, `context/answer-logic.md` "FAQ RAG 매칭 방식"에는 이미 "질문 표현(Q) 필드 전체(운영자가 입력한 여러 발화 예시 전부)를 Concept의 동의어·예문 전체 인덱싱 방식과 동일하게 인덱싱한다"고 명시돼 있어 설계 스펙과 목업 UI가 어긋나 있었음(2026-07-06에 출처·유의사항 필드 불일치는 고쳤지만 이 Q 구조 문제는 당시 놓침). "/" 구분자 방식은 실제 구현 시 여러 발화를 한 덩어리 텍스트로 임베딩하게 돼 매칭 정확도가 떨어지는 기능적 문제로 이어질 수 있음을 확인.

`06_card-editor-concept.html`의 동의어 칩 입력(`chip-area`/`chip-input`/`chip-remove`, Enter로 추가)을 그대로 재사용해 Q 필드를 배열 구조로 전환 — 등록 패널·목록 테이블(첫 문구+나머지 개수 배지)·상세 모달(칩 목록 표시)·검색 필터 전체 반영. 샘플 데이터 FAQ005도 실 콘텐츠(FAQ-001)와 대조해 누락돼 있던 세 번째 발화("폴립절제 보험금 받을 수 있나요?")를 추가해 정확히 일치시켰다.

**수정 파일:** `mockups_v2/19_faq-rag.html`
