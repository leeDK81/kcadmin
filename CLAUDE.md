...
상세·논의 경과는 `contents/decisions.md` "미결 항목" 표가 유일한 진실원 — 완료된 항목의 히스토리는 `contents/CHANGELOG.md`에 있으므로 여기 재서술하지 않는다. **2026-07-30(2차) 갱신 기준 실제 미결 4건:**

1. **FAQ-008~010 플레이스홀더**(2026-07-04) — 상담취소·아이지넷 소개·마이데이터 연동 안내 3건, 실제 서비스 정책 확정 후 교체 필요.
2. **결과 카드 경각심 강화 방향**(2026-07-20) — `14_card-hook-sample.html` Before/After 샘플 검토 대기, 실 콘텐츠 미반영.
3. **A9617(T18) 개념 충돌**(2026-07-30) — 내부 기준금액 엑셀(v1.2·v2.1 모두)이 A9617에 현재 값(350만원, 실측치)과 크게 다른 값(5,000만원, "비급여 버퍼" 개념, 근거등급 D)을 제시하나 v2.1 스스로도 신뢰도 낮음을 자인 — 현재는 실측치 유지, 개념 재정의는 급하지 않으면 보류. 상세는 `contents/decisions.md` 참조.
4. **mockups_v2/ Risk-type 제목 미반영**(2026-07-29) — T03·T04(반쪽형)·T11(공백형) 개명이 서비스 트랙 7개 파일(04_risk-type-list.html 등)에는 아직 안 감. 별도 요청 시 반영.

(연령대 보정계수 도입 검토(2026-07-30 1차 등록)는 같은 날 2차로 종료 처리됨 — 후속 엑셀 v2.1이 임의 연령배수 접근 자체를 철회했기 때문.)

**세션 시작 시 확인 순서:** 이 4개 항목만 보면 됨 → 우선순위/진행 여부는 사용자에게 확인. 작업 완료 시 이 목록에서 항목을 지우고 CHANGELOG.md에 기록할 것(여기 다시 서술하지 말 것 — 이 문단이 매 세션 로드되므로 비대해지면 안 됨).
