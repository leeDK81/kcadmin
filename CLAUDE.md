...
**사전 테스트 UX 2건 개선 (2026-06-30):**
- **PROMAGE 조건**: 위험/주의/양호/미설정 등급 버튼 → `✓ 충족 / ✗ 미충족` 토글 (MYDATA와 동일). 조건에 기댓값 이미 명시 → 등급 재선택 불필요.
- **RAG 결과 설정**: 약관 RAG 제거. 약관 RAG는 사용자 실제 보험 계약(MYDATA) 없이 테스트 불가 — 배포 판단 관문에 검증 불가 항목 있으면 안 됨. `ragMockState`: `{yakgwan,faq}` → `'faq'\|'llm'`. 결과 흐름: KC평가 → FAQ RAG → LLM Fallback (3단계).
