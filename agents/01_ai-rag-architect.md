...
3-1. **RAG 레이어 아키텍처 (2026-06-21 확정)**
   ...
   ```
   사용자 질문
   ├─ Case 0: 데이터 조회형 발화 감지 → 마이데이터 직접 조회 응답 (콘텐츠 매칭 없음)
   └─ Case 0 미감지
   ...
   > Playbook 매칭은 위 분기와 무관하게 항상 별도로 병행 실행되며, 감지되면 어느 경로든 CTA가 가산된다(Case 0 포함).

   → **Case 0~4 매트릭스 및 RAG 역할 분리 상세:** `context/answer-logic.md`
...
4-2. **Risk-type 다중 감지 시 노출 우선순위**
   복수의 Risk-type이 동시 감지될 때 어떤 순서로 노출할지 결정하는 로직을 구현한다(가중치 없음, 순위 기반).
   → **우선순위 기준 상세:** `context/card-types.md`
...
| §3-6 | ~~Amplitude 유저 속성 조건 연동~~ → 폐기됨. 현재 Rule 조건 소스는 `context/card-types.md` 참조 |
| §3-7 | 리스크 유형 조건 사전 테스트 검증 |
| §3-8 | 프롬에이지 민감정보 처리 원칙 — 출력 등급명 표기 규칙은 `guides/copywriting.md` 참조 |
