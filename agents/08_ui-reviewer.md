...
| Risk-type 편집기 중요도 | 중요도 라디오 그룹(높음/보통/낮음 — 가중치 숫자 없음) 존재. 기본값 "보통". 힌트 문구에 "①중요도 서열(높음>보통>낮음) → ②선택 조건 충족 개수(시스템 자동) → ③최근 배포순" 우선순위 설명 있는가? 가중치(×3/×2/×1) 표기 완전 삭제 확인 |
...
| 항목 | 기준 |
|---|---|
| 금지어 전수 | `guides/copywriting.md` 금지어 목록 기준 — 화면·레이블·placeholder 모두 확인 |
...
| 항목 | 기준 |
|---|---|
| 승인완료 카드 액션 | "상세 보기" + "캔버스에서 연결" 버튼만 표시. "라이브 전환" 버튼 없어야 함 (라이브 전환은 사전 테스트 패널 푸터에서 처리) |
