...
| **"전체 진단해줘"·다건 매칭 케이스 — 답변 포맷 설계 완료, 카드 저작만 남음** | 2026-07-05 논의로 처리 방식은 확정됨(`context/answer-logic.md` "다건 매칭 시 답변 처리" 참조): 활성 Risk-type 23개 전체를 `risk_type_refs`로 연결한 전용 Concept 카드(가칭 `CN-ALL`)를 신설해 기존 N:N 구조로 처리하고, 매칭 N≥2일 때는 요약형(도메인 그룹핑+항목별 1줄+정렬은 기존 우선순위 로직 재사용) → 개별 지목 시 기존 단건 심층 포맷으로 전환하는 2단 구조로 답변한다. `contents/html/08_ai-preview.html`에 개념 검증용 예시(경로 A·B) 추가 완료 | 남은 작업: ① `CN-ALL` 카드 실제 저작(컨텐츠 트랙) ② PROMAGE 선택조건 "짧은 표시" UI 형태 확정 ③ 요약형 답변의 실제 LLM 프롬프트 반영은 개발팀 몫 | 2026-07-05 |
