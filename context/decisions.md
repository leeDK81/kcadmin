...
| FAQ RAG 매칭 세부값 — 기준값·Top-K·카테고리 우선순위 | 매칭 방식(임베딩 유사도, Concept·Playbook과 동일 엔진 재사용)은 확정했으나 ①기준값 수치를 Concept·Playbook과 공유할지 별도로 둘지, ②최고점 1개만 쓸지 Evidence처럼 Top-K 후 LLM 선택 방식으로 할지, ③카테고리 A(판단노하우)·B(서비스안내) 동시 매칭 시 가중치를 둘지 미정 | 개발팀 결정 필요 (matching-policy.md 기존 미결 항목과 동일 성격) | 2026-07-05 |
| **결과 카드 컴포넌트 실장** | "전체 진단해줘"·다건 매칭 케이스 답변 포맷(카드 우선 UI) 정책·컨텐츠 저작은 완료됨(`context/answer-logic.md` "매칭 시 답변 처리 — 카드 우선 UI로 통일" 참조, `contents/html/08_ai-preview.html` 정적 목업 완성). 실제 앱 컴포넌트(`.app-result-cards` 등)는 아직 `mockups_v2/` 서비스 트랙에 구현 안 됨 | 개발팀 구현 필요 | 2026-07-05 |
