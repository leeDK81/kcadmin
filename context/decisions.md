...
### 캔버스 연결 방향 (CONNECT_RULES — 2026-06-22 최종 확정)

```javascript
const CONNECT_RULES = {
  concept:  ['risk'],    // 필수 (최소 1개) — 미연결 시 KC 체인 미진입
  risk:     ['rule'],    // 필수 (최소 1개)
  rule:     ['evidence', 'policy'],  // evidence 필수 / policy 선택
  evidence: [],          // 단말
  policy:   [],          // 단말
  playbook: [],          // 단말 (독립 체인)
};
...
## Concept → Risk-type: 1:N 다중 선택 (2026-06-10 확정 / 2026-06-22 필수 재확정)
...
- **필수 연결 (2026-06-22 확정):** Concept은 반드시 1개 이상의 Risk-type과 연결해야 KC 체인에 진입. 연결 없으면 KC 체인 미진입 — Case 3(RAG→Fallback) 또는 Case 4(Playbook 감지) 분기로 처리됨.
- ~~선택사항 — 연결 없어도 동의어 매칭 유효~~ **[삭제 2026-06-22]** Concept Standalone 기능 없음. 완전 제거됨.
...
## Rule 조건 빌더 3-source (2026-06-08 확정 / 2026-06-22 조건 구조 재확정)
...
**조건 구분 — 필수 / 선택 (2026-06-22 확정):**

각 조건 행은 필수 또는 선택으로 지정한다.
...
**프롬에이지 미연동 대응:**
- 프롬에이지 연동을 하지 않은 사용자는 Promage 소스 조건 평가 불가
- Promage 조건은 원칙적으로 **선택 조건**으로 설정 권장
- 운영자가 Promage 조건을 필수로 설정한 경우 → 미연동 사용자는 해당 Rule 발동 안 됨 (의도적 설계로 허용)
...
| Card ② | 리드 전환 레이어 — 발화 키워드(최소 3개) + 전환 액션(기본 버튼 consult 필수 + 추가 버튼 N개) + Standalone 답변 가이드(선택사항 — 비워두면 Clark 기본 안내 문구 사용) |
...
**Playbook 라이브 전환 (2026-06-22 확정):** Playbook은 캔버스 연결 없이 approved 상태에서 직접 "라이브 전환" 가능 (독립 체인). KC 카드 연결 불필요.

---

## 면책 문구 관리 — Policy 카드 (B안 확정 / 2026-06-22 필드 재확정)

Clark 앱 면책 고지 문구는 **Policy 카드 "Clark 앱 표시 문구" 필드**로 관리.
준법감시인 승인 후 KC API를 통해 앱에 즉시 반영. 앱 코드 하드코딩 금지.

**Policy 편집기 확정 필드 2개 (2026-06-22 확정):**
...
## Risk-type 다중 감지 시 노출 우선순위 (2026-06-22 재확정)

**배경:** 사용자 보험 진단 결과 복수 Risk-type이 동시에 감지될 때 노출 순서 기준 필요.

**설계 원칙:**
- Rule은 "이 사용자가 해당 리스크인가" 진단만 담당 (발동 여부 판단)
- 우선순위는 Rule 개수·조건 수와 무관 — 서비스 전략(중요도)과 사용자 개인화(선택 조건)로만 결정
- Rule 충족 개수를 우선순위에 반영하면 Rule을 많이 연결한 카드가 유리해지는 구조적 문제 발생 → 제거

**우선순위 계층:**

| 순위 | 기준 | 의미 |
|---|---|---|
| 1순위 | 중요도 | 운영자 판단 — 서비스 전략상 어떤 리스크가 더 중요한가 |
| 2순위 | 선택 조건 충족 개수 | 이 사용자에게 얼마나 강하게 해당하는가 (개인화) |
| 3순위 | 카드 코드 오름차순 | 순수 타이브레이크 (T01 우선) |
...
**운영자 설정:** Risk-type 편집기 "중요도" 라디오 (높음/보통/낮음). 2·3순위는 시스템 자동 계산.
...
연결 가능 카드: 승인완료(approved)·라이브(active) — 임시저장·승인요청은 잠금 처리 (opacity:0.4, pointer-events:none). 승인완료 카드 연결 = 준비 연결(pending edge, 점선 파랑)

---

## 2026-06-22 확정 사항

### Concept → Risk-type: 연결 필수 (2026-06-22 재확정)

**결정:** Concept 카드에서 Risk-type 연결은 **필수(required)**. 연결 없으면 KC 체인 진입 불가.

**이유:** Risk-type 미연결 Concept은 KC 매칭 체인 출발점이 될 수 없음. Case 3·4로 분기.

**삭제 표현:** "연결 없이도 동의어 매칭은 유효" — 오해 유발 금지. 코드/문서에서 완전 삭제.

**Concept 미매칭 시 분기:**
- Case 3 (Playbook 미감지): RAG(약관→FAQ) → Fallback 생성형
- Case 4 (Playbook 감지): Standalone 가이드 주입 + CTA 버튼

> 이전 결정 (2026-06-10): "선택사항 — 연결 없어도 동의어 매칭 유효" → **폐기. 이 항목으로 대체.**

---

### Concept Standalone 완전 제거 (2026-06-22 재확정)

**결정:** Concept 카드에 Standalone 기능 없음. 구 기획 흔적 코드·문서 전면 삭제 대상.

**이유:** Case 1~4 매트릭스가 모든 시나리오를 커버. 미매칭 케이스는 Playbook(Case 4)이 담당.

**Case 1~4 정의 (KC매칭 × Playbook감지 매트릭스 — 최종):**

| Case | KC Concept 매칭 | Playbook 감지 | 동작 |
|---|---|---|---|
| Case 1 | O | X | KC 구조화 답변 |
| Case 2 | O | O | KC 구조화 답변 + CTA 버튼 |
| Case 3 | X | X | RAG(약관→FAQ) → Fallback 생성형 |
| Case 4 | X | O | Standalone 가이드 주입 + CTA 버튼 |

**구 기획 삭제 대상:** Concept 편집기 Standalone/Linked 라디오, Standalone 관련 Concept 필드 일체.

> 이전 결정 (2026-06-10): Concept 편집기 Standalone 라디오 제거 기록 있음. Standalone 기능 자체의 완전 부재 명시 추가.

---

### Policy 카드 필드 2개 확정 (2026-06-22 재확정)

**결정:** Policy 편집기 확정 필드: **Policy 이름(name)** + **Clark 앱 표시 문구(appDisplayText)** 2개만.

**이유:** 불필요한 필드 제거로 운영 단순화. 운영자 실무 목적(면책 고지 문구 직접 작성)에 집중.
...
**게이트:** Clark 앱 표시 문구 비어있으면 검수 요청 불가.

**승인 2단계:** 도메인 검수자 → 준법감시인.

> 이전 결정 (2026-06-17): Policy 편집기 기본정보 카드 순서에 "규제 문서명 → 적용 범위 → 핵심 조항 요약…" 기재. **해당 필드 목록 폐기. 이 항목으로 대체.**

---

### Playbook Standalone 선택사항 확정 (2026-06-22 재확정)

**결정:** Playbook의 "Standalone 답변 가이드" textarea는 선택사항. 비워두면 Clark 기본 안내 문구 사용.

**이유:** 등록 진입장벽 제거. 최소 키워드·CTA 버튼만 있으면 Playbook 등록 가능.

**삭제 표현:** "최소 20자 필수" — 코드·문서에서 완전 삭제.
...
**추가 확정:** approved 상태에서 캔버스 연결 없이 직접 "라이브 전환" 가능 (Playbook은 독립 체인 단말, 캔버스 연결 불필요).

> 이전 결정 (2026-06-08): Card ② "Standalone 답변 가이드(20자 이상 필수)" → **"20자 이상 필수" 삭제. 선택사항으로 변경.**

---

### Risk-type 우선순위 가중치 완전 제거 (2026-06-22 재확정)

**결정:** 중요도 가중치(높음×3/보통×2/낮음×1) 개념 완전 제거. **서열(높음>보통>낮음)만 사용.**

**이유:** 가중치 곱셈 없는 서열 방식으로 단순화. 운영자·코드 모두 직관적으로 이해 가능.
...
| 3순위 | 카드 코드 오름차순 | 타이브레이크 (T01 우선) |

**삭제 대상:** `weight: 3/2/1`, `importance * weight` 연산, "×3/×2/×1" 표기 — 코드·문서 전면 삭제.

> 이전 결정: "중요도×Rule충족수→조건수합계→코드 오름차순" 및 "높음×3/보통×2/낮음×1" 가중치 방식 → **가중치 곱셈 방식 전면 폐기. 이 항목으로 대체.**

---

### Rule PROMAGE 기본값 = 선택 (2026-06-22 확정)

**결정:** Rule 조건 빌더 소스별 `required` 기본값:

| 소스 | 기본 required | 이유 |
|---|---|---|
| MYDATA | `true` (필수) | 마이데이터 미연동 사용자는 KC 진단 불가 — 필수 전제 |
| PROMAGE | `false` (선택) | 프롬에이지 미연동 사용자 대응. 미충족 시 Rule 발동 차단 방지 |
| PROFILE | `true` (필수) | 프로파일 정보는 상담 시 항상 수집 |

**운영자가 PROMAGE 조건을 필수로 변경하는 것은 허용** — 의도적 설계 (미연동 사용자 Rule 발동 안 됨).

**ConditionRow 속성:** 각 조건 행에 `required: boolean` 속성 존재. 편집기에서 필수/선택 토글 가능.

---

### 금지어 추가: 임계값 (2026-06-22 확정)
...
**전체 금지어 현황 (2026-06-22 기준):**
- 비활성/비활성화 → 잠김/잠금 처리/연결 불가
- 시뮬레이션 → 사전 테스트
- 임계값 → 기준값/판단 기준
- 런타임, 파라미터, 라우팅, 매핑, 상/중/하
- 3-source (화면/문서 노출 금지)
- MYDATA → 마이데이터 (화면 표기. 코드는 MYDATA)
- Promage → 프롬에이지 (화면 표기. API 코드는 PROMAGE)

---

### CONNECT_RULES 최종 확정 (2026-06-22)
