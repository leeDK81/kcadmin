...
## FAQ RAG 역할 정의 및 19_faq-rag.html 화면 확정 (2026-06-21 확정)

### FAQ RAG = Clark 서비스 전용 Q&A (약관 RAG와 역할 분리)

**결정:** FAQ RAG는 "Clark 서비스 전용 Q&A"만 등록. 일반 보험 개념 설명·약관·보장 관련 Q&A 등록 금지.

**이유:** 약관 관련 질문은 약관 RAG가 가입 상품 약관 원문을 직접 검색하므로 FAQ RAG에 중복 등록 불필요.

| RAG 유형 | 담당 영역 | 관리 방식 |
|---|---|---|
| 약관 RAG | 가입 상품 약관 원문 검색·인용 | 크롤러 자동 파이프라인 |
| Clark 서비스 FAQ RAG | Clark 앱 사용 안내, 보닥 플래너 연결, 서비스 정책 등 서비스 고유 Q&A | 운영자 직접 등록 → 검수자 승인 |

**"약관에서 Q&A 초안 생성" 기능 삭제 (확정):**
- 기존 계획했던 LLM 기반 약관→Q&A 자동 초안 생성 기능 제거
- 이유: 약관 RAG가 직접 원문 검색하므로 FAQ 형태 변환 불필요
- 현재: 운영자가 Clark 서비스 전용 Q&A를 직접 입력만 가능

### 19_faq-rag.html 화면 스펙 (2026-06-21 확정)

**역할:** Clark 서비스 전용 FAQ Q&A 등록·검수·관리

**주요 기능:**
- 목록 테이블: 상태 필터(전체/검수대기/인덱스등록됨/반려됨) + 검색
- 직접 등록 패널: 인라인 패널 (Q·A 입력)
- 상세 모달: 수정·승인·반려·재검수 액션
...
| 초안 | 운영자 작성 중 |
| 검수대기 | 검수자 검토 요청 후 |
| 인덱스 등록됨 | 검수자 승인 → RAG 인덱스에 반영됨 |
| 반려됨 | 검수자 반려 (사유 포함) |

**등록·승인 주체:**
- 등록 주체: 운영자 (직접 입력만, LLM 초안 생성 없음)
- 승인 주체: 검수자

**등록 금지:** 약관·보장·담보 관련 Q&A (약관 RAG가 자동 처리)
...
체인 순서가 아닌 **카드 주체별** 기준으로 정의. 각 카드의 유입(←)·유출(→) 기준을 따르면 체인 순서는 자연히 성립됨.

| 연결 | 출발 카드 기준 (유출) | 도착 카드 기준 (유입) |
|---|---|---|
| Concept → Risk-type | 1:N (제한 없음, 선택) | N:1 (여러 Concept 허용) |
...
**체인 중간에 카드 추가 시:** 새 카드의 유입 조건(앞 카드 → 연결 추가) → 유출 조건(뒤 카드로 연결 추가) 순서로 처리.
...
| 연결 안내 | Playbook은 다른 카드와 직접 연결 없음 (Phase 1.5+ 검토 중 — 리드 스코어링 스펙 확정 시 결정). 정보 표시용 카드, 입력 필드 없음 |
...
| 순위 | 기준 | 설명 |
|---|---|---|
| 1순위 | 중요도 가중치 × Rule 충족 개수 | 운영자 가중치(×3/×2/×1) × 충족된 Rule 수 |
| 2순위 | 충족된 Rule들의 조건 수 합계 | 동점 시 — 조건 많은 Rule이 충족될수록 더 정밀한 매칭 |
| 3순위 | 카드 코드 오름차순 | 최종 타이브레이크 (T01 우선) |

**중요도 가중치 (3단계):**

| 중요도 | 가중치 | 의미 |
|---|---|---|
| 높음 | 3 | 핵심 진단 유형 — 다른 유형과 동시 감지 시 선노출 |
| 보통 | 2 | 일반 진단 유형 — 기본값 (미설정 시 자동 적용) |
| 낮음 | 1 | 보조 진단 유형 — 상위 유형 노출 후 후순위 안내 |

**운영자 설정:** Risk-type 편집기에 "중요도" 라디오 (높음/보통/낮음) 추가. 2·3순위는 시스템 자동 계산.
