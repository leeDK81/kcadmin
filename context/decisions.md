...
| **"전체 진단해줘" 케이스 — 카드 기반 답변 불가능** | "내 보험 전체 진단해줘" 같은 포괄적 질의는 `contents/04_concept/concepts.md`에 대응하는 Concept 자체가 없어 Concept 미매칭 → RAG/FAQ/LLM Fallback으로 처리됨. 현재 구조(Concept 1개 매칭 → 해당 Risk-type만 평가)로는 23개 Risk-type을 전부 평가해 다건 요약 답변을 만드는 경로가 없음. 여러 Risk-type이 한 Concept으로 동시 감지되는 기존 "다중 감지 우선순위"(`13_answer-logic.html` Step 2-B)와는 별개 문제 — 그건 "하나의 질문이 여러 Risk-type에 걸릴 때 노출 순서"이고, 이건 "애초에 전체 스캔을 트리거하는 인텐트 자체가 없음" | 전용 Concept/인텐트 신설 여부 + 다건 요약 답변 포맷(리스트형 등) 설계 필요 — PO 논의 후 결정 | 2026-07-05 |
