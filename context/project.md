...
> KC Admin **서비스 기획 트랙**(`mockups_v2/` 어드민 UI 목업) 전체 구조를 이해하는 데 필요한 맥락. `CLAUDE.md` 에이전트 구성 표 기준 계층2 참조 목록에 이 파일이 포함된 에이전트는 **06(기획검수)·07(보험도메인전문가)**뿐이다(2026-07-07 정정 — 이전엔 "01·02·03·06·07"로 잘못 표기돼 `CLAUDE.md`의 실제 표와 어긋나 있었다).
...
**2026-07-06 추가 수정 (2026-07-07 이 노트 갱신):** Case 0 신설·정정에 따라 `01_glossary.html`·`02_card-purpose.html`·`08_screen-policy.html` 3개 파일 추가 수정(Case 매트릭스·"Concept·Playbook 동시 건너뜀" 오표현 정정 등). 상세는 `context/decisions.md` 2026-07-06 항목 참조.
