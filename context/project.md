...
- Risk-type → Rule (필수, 정확히 1개 — 1:1 전속)
...
## 사용자 데이터 상태 4종 — 선행 분기 (2026-06-26 확정)

Clark이 profile/mydata/promage를 null 포함 그대로 전달 → KC Engine이 null 여부로 상태 판단. 별도 `user_state` 파라미터 불필요.

| 상태 | PROFILE | MYDATA | PROMAGE | 약관 RAG | 답변 우선순위 |
|---|---|---|---|---|---|
| ① 미가입/미로그인 | null | null | null | 불가 | FAQ RAG → LLM |
| ② 회원 + 마이데이터·프롬에이지 미연동 | 있음 | null | null | 불가 | KC(PROFILE만) → FAQ RAG → LLM |
| ③ 마이데이터 연동 + 프롬에이지 미이용 | 있음 | 있음 | null | 가능 | KC(PROMAGE 스킵) → 약관 RAG → FAQ RAG → LLM |
| ④ 메인 서비스 이용 (전체 데이터) | 있음 | 있음 | 있음 | 가능 | KC(전체 평가) → 약관 RAG → FAQ RAG → LLM |

앞 단계 성공 시 뒤 단계 호출 없음(분기). Playbook CTA는 어느 경로든 키워드 감지 시 가산. 상세 → `context/answer-logic.md`

---

## 사용자 질의 답변 케이스 4종 — 상태 ③④ 전용

> Case 1~4는 **MYDATA 있는 상태 ③④**에만 적용. 상태 ①②는 위 "사용자 데이터 상태 4종" 분기를 따름.

| 케이스 | KC Concept 매칭 | Playbook 키워드 감지 | Clark 출력 |
|---|---|---|---|
| Case 1 | ✓ 매칭 | — | KC 구조화 답변 (RAG·LLM 호출 없음) |
| Case 2 | ✓ 매칭 | ✓ 감지 | KC 구조화 답변 + CTA 버튼 |
| Case 3 | — 미매칭 | — | 약관 RAG → Clark FAQ RAG → Fallback 제한 생성형 (18_system-settings 설정 적용) |
...
## RAG 아키텍처 (2026-06-21 확정 / 2026-06-26 상태별 분기 추가)

KC 카드 미매칭(Case 3/4) 발생 시 RAG 레이어가 동작한다.

```
사용자 질문
├─ KC 카드 매칭 → 구조화 답변 (Case 1/2) — RAG·LLM 호출 없음
└─ KC 미매칭 (또는 상태①② — KC 전체/부분 불가)
   ├─ 약관 RAG (상태③④만 가능 — MYDATA 없으면 건너뜀)
   │    └─ 유사도 ≥ 0.75 → 약관 RAG 기반 답변 (끝)
   └─ Clark 서비스 FAQ RAG: 운영자 직접 등록 → 즉시 인덱스 등록
        ├─ 결과 있음 → FAQ RAG 기반 답변 (끝)
        └─ 결과 없음 → Fallback 제한 생성형 (18_system-settings 설정 적용)

+ Playbook 병렬: 어느 경로든 키워드 감지 시 CTA 가산
```
...
| Clark 서비스 FAQ RAG | Clark 앱 사용 안내·서비스 정책·보닥 플래너 연결 등 서비스 고유 Q&A | 운영자 직접 등록 → 즉시 인덱스 등록 (19_faq-rag.html) | 약관·보장 관련 Q&A (약관 RAG가 처리) |

**FAQ RAG 등록 원칙:**
- 등록 주체: 운영자 직접 입력 (승인 프로세스 없음, LLM 초안 자동 생성 없음)
- 상태 2종: 초안 / 인덱스 등록됨
