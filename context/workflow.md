...
- **`<div class="nav-section">` 섹션 레이블 완전 제거** — `nav-divider`만 사용 (2026-06-25 결정)
- **`.nav-header` 신규 도입으로 위 원칙 일부 수정 (2026-07-07):** "시스템 설정"이 자체 페이지 없이 하위 항목만 묶는 순수 그룹 라벨이 되면서, 클릭 불가능한 텍스트 라벨이 다시 필요해짐. `nav-section`(제거된 구 클래스)과 달리 `.nav-header`는 상위 링크가 사라지고 하위 항목만 남는 경우에 한정해서만 쓴다 — 일반적인 구분에는 여전히 `nav-divider`만 사용.
- **사이드바 순서 (2026-07-07 갱신):**
- 조회형 메시지 관리(신규, 카드 라이브러리 바로 위) → 카드 라이브러리 그룹 (divider) → 검수·승인 → **연결·테스트·배포** → FAQ RAG(2026-07-07 "FAQ Q&A"에서 이름 변경, 시스템 설정 하위에서 최상위로 이동) → 시스템 설정(`.nav-header` — LLM 응답 가이드 · 유사어 관리) (divider) → 참조 섹션
- 상세 마크업은 `guides/ux-patterns.md` "사이드바(전 파일 공통)" 참조
