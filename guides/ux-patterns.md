...
<!-- review 카드 (2026-07-07 정정 — 이전엔 이 예시가 빠져 있었고 위 approved 항목이 "review 카드"로 잘못 표시돼 있었음) -->
<span class="gc-status-badge gc-badge-review">승인요청</span>
```
...
.gc-badge-approved { background:rgba(26,74,154,.12); color:var(--status-approved) }
.gc-badge-review   { background:rgba(184,119,34,.1); color:var(--status-review) }
...
다중 Risk-type이 동시 감지될 때 노출 순서를 결정하는 중요도 설정 UI. **우선순위 규칙(가중치 없는 서열 방식)은 `context/card-types.md`가 단일 진실원**이다(2026-07-07 정리 — 이전엔 우선순위 3단계 설명이 여기 그대로 복사돼 있었음). 아래는 그 규칙을 반영한 HTML 패턴이다.
...
**소스별 기본값은 `context/card-types.md`가 단일 진실원**이다(2026-07-07 정리).
...
**필드 규칙(선택 여부·최소 키워드 수 등)은 `context/card-types.md`가 단일 진실원**이다(2026-07-07 정리). 아래는 그 규칙을 반영한 HTML 패턴이다.
...
**UI 특이사항:** approved 상태에서 캔버스 연결 없이 직접 "라이브 전환" 가능(다른 카드 타입과 다른 예외 흐름).
